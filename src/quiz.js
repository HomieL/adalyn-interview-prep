...
export function renderQuizSetup() {
  ...
  document.getElementById('mainContent').innerHTML = `
    <div class="quiz-setup">
      ...
      <p>${t(`选择题型，AI 生成 10 道面向${TARGET.roleZh}面试的定制题目，每题含提示和解析。`, `Choose a question type. AI generates 10 custom ${TARGET.short} interview questions, each with a hint and explanation.`)}</p>
      <div class="mode-grid">
        ...
        <div class="mode-card${selectedMode === 'scenario' ? ' selected' : ''}" onclick="pickMode(this,'scenario')">
          ...
          <div class="md">${t('技术选型·调试排错·性能分析·项目实战等场景', 'Technology choices, debugging, performance analysis, and real project scenarios')}</div>
        </div>
      </div>
...
export async function startQuiz() {
  ...
  showLoading(t('正在生成题目…', 'Generating questions…'), t(`生成 10 道${TARGET.shortZh}级别${mode === 'concept' ? '概念' : '场景'}题`, `Generating 10 ${TARGET.short}-level ${mode === 'concept' ? 'concept' : 'scenario'} questions`))
  try {
...
async function genQuiz(c, mode) {
  const mDesc = mode === 'concept'
    ...
    : '场景实战题（技术选型、调试排错、性能分析、在项目中的实际应用）'
  ...
  const makePrompt = (batch, start, angleHint) =>
    `请为${TARGET.roleZh}面试出第 ${start + 1}-${start + batch} 道关于「${c.name}」的${mDesc}，共出 ${batch} 道。${angleHint}
...
- 实习面试难度：考查对原理的真正理解，而不是死记硬背；不要求大规模系统设计经验
